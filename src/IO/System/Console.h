...
/**
 * @file Console.h
 * @brief 提供一个接近 C# 控制台体验的跨平台 C++17 工具（支持颜色、标题、Eigen 输出）。
 *
 * @details
 * 该文件为纯头文件实现，直接包含即可使用。
 *
...
private:
    ...
    static void applyColor() {
        ...
        std::string esc;
        esc.reserve(14);
        if (s_fg != ConsoleColor::Default) esc += ansiEsc(s_fg, false);
        if (s_bg != ConsoleColor::Default) esc += ansiEsc(s_bg, true);
        if (!esc.empty()) {
            std::cout << esc;
            std::cout.flush();
        }
...
public:
    ...
    explicit ConsoleColorScope(ConsoleColor fg,
        ...
        if (fg != ConsoleColor::Default) Console::SetForegroundColor(fg);
        if (bg != ConsoleColor::Default) Console::SetBackgroundColor(bg);
    }
