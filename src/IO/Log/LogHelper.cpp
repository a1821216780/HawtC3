	...
	namespace IO
	{
		namespace Log
		{
			...
			void LogHelper::EndInformation(double tfinal, double dt, const std::chrono::steady_clock::time_point &start_time)
			{
				...
				std::string timeString = (elapsedTime > 60.0) ? std::to_string(elapsedTime / 60.0) + " min" : std::to_string(elapsedTime) + " sec";
									 ...
									 const std::string &title, int outtime, const std::string &FunctionName)
			{
				...
				std::string Ntitle = title;
				std::string Nrelmessage = relmessage;
